...
frm.addEventListener('submit', (e) => {
    ...
    frm.inPaciente.focus()               //posiciona cursor no campo
})

//Aciona um "ouvinte" para o evento click no btUrgencia que está no form
frm.btUrgencia.addEventListener('click', () => {
    //verifica se as validações do form estão ok (no caso, pacientte is required)
...
frm.btAtender.addEventListener('click', () => {
    ...
    if (pacientes.legth == 0){
        alert("Não há pacientes na lista de espera")
        frm.inPacientes.focus()
        return
    }
    const atender = pacientes.shift()    //remove do inícioda fila (e obtém nome)
